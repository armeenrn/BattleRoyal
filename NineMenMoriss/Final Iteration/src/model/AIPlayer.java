...
public class AIPlayer extends Player {
	...
	public AIPlayer(String name, int num, boolean goFirst, Board gameBoard) throws Exception {
	...
	/**
	 * Logic that checks if the AI can prevent the opponent from forming a mill by
	 * removing a stone.
	 * 
	 * @param board          Gameboard
	 * @param opponentPlayer
	...
	public Stone findStopMillByRemove(Board board, Player opponentPlayer) {
		...
		for (Square square : board.getSquares()) {
			for (Line line : square.getLines()) {
				...
				// checks if the line has exactly 1 empty point and 2 points occupied by Human
				// Player
				...
				if (numEmptyPoint == 1 && numHumanPoint == 2) {
					if ((opponentPlayer.getNumberOfPlacedStones() < 9)
							|| opponentPlayer.getNumberOfTotalStones() == 3) {
						// the opponent can place or jump; remove any two of the stones to prevent a
						// mill
						for (Point point : line.getPoints()) {
						...
						// the opponent can only move adjacent; remove only if the opponent can form a
						// mill in one turn
	...
	public Point lookForBestMove(Board board) {
		...
		if ((getNumberOfPlacedStones() < 9) || (getNumberOfTotalStones() == 3)) {
			...
			if (!foundMove) {
				...
				if (!foundMove) {
					// cannot stop opponent from forming a mill, or opponent cannot form a mill next
					// turn
	...
	/**
	 * Logic that checks if the AI can form a mill by moving a stone The AI does not
	 * have restrictions on movement
	 * 
	 * @param board Gameboard
	...
	public boolean checkMillByPlace(Board board) {
		...
		for (Square square : board.getSquares()) {
			for (Line line : square.getLines()) {
				...
				// check if the line has exactly 1 empty point and 2 points occupied by AI
				// Player
				for (Point point : line.getPoints()) {
					if (point.getOccupiedPlayer() == 0) {
						pointToTest = point;
					...
					if (getNumberOfPlacedStones() < 9) {
						...
						for (Stone currentStone : getStones()) {
							if (!currentStone.getDead()) {
								...
								// if the live stone tested was not on the current line it will move to the
								// point
								if (!wasStoneOnLine) {
	...
	public boolean checkStopMillByPlace(Board board) {
		...
		for (Square square : board.getSquares()) {
			for (Line line : square.getLines()) {
				...
				// check if the line has exactly 1 empty point and 2 points occupied by Human
				// Player
				for (Point point : line.getPoints()) {
					if (point.getOccupiedPlayer() == 0) {
						pointToTest = point;
				...
				if (numEmptyPoint == 1 && numHumanPoint == 2) {
					// if this is placing stage there is no need to check for moving stones on
					// current line
					if (getNumberOfPlacedStones() < 9) {
						pointToMoveTo = pointToTest;
						return true;
	...
	/**
	 * Logic that checks if the AI can move a stone to prepare for a mill the turn
	 * after The AI does not have restrictions on movement
	 * 
	 * @param board Gameboard
	 * @return Indicates whether a move was found
	 */
	...
	/**
	 * Logic that checks if the AI can form a mill by moving a stone The AI can only
	 * move a stone to its adjacent point
	 * 
	 * @param board Gameboard
	 * @return Indicates whether a move was found
	 */
				...
				if (numEmptyPoint == 1 && numCompPoint == 2) {
					...
					for (Point adjacentPoint : adjacentPoints) {
						...
						if (adjacentPoint.getOccupiedPlayer() == 2) {
							// check if the stone is not on the current line; if it is not, it moves to the
							// empty point
							try {
								if (!(line.doesLineContain(adjacentPoint.getOccupiedStone()))) {
	...
	/**
	 * Logic that checks if the AI can move a stone to prepare for a mill the turn
	 * after The AI can only move a stone to its adjacent point
	 * 
	 * @param board Gameboard
	 * @return Indicates whether a move was found
	 */
	...
	/**
	 * Case 1 of "two steps from mill" when AI can only move adjacent When there are
	 * two computer stones and one empty point on the line, check adjacent points of
	 * the empty point
	 * 
	 * Case 1-1: The adjacent point is empty, and the adjacent of that adjacent
	 * point is occupied by AI The AI Player can move that stone to the adjacent
	 * point to form a mill the turn after by moving the stone to the empty point.
	 * 
	 * Case 1-2: The adjacent point is occupied by AI, and the adjacent of the
	 * adjacent point is occupied by AI Case 1-2-1: The empty point was the midpoint
	 * of the line The stone in the adjacent point was in an endpoint of the line;
	 * move that to midpoint AI can form a mill the turn after by moving the stone
	 * in the adjacent to adjacent point to the now empty endpoint
	 * 
	 * Case 1-2-2: The empty point was an endpoint of the line Check if the stone in
	 * the adjacent to adjacent point was on the line. If not, the stone in the
	 * adjacent point can move to the empty endpoint. AI can form a mill the turn
	 * after by moving the stone in the adjacent to adjacent point to the now empty
	 * midpoint
	 * 
	 * @param pointToTest Point to check if a stone can make a logical choice to
	 *                    move here
	 * @param line        Line that pointToTest belongs to and will be called to
	 *                    test
	 * @return Indicates whether a move was found
	 */
	...
	/**
	 * Case 2 of "two steps from mill" when AI can only move adjacent When there are
	 * one computer stone and two empty points on the line, check adjacent points of
	 * the point occupied by AI
	 * 
	 * Test 1: An adjacent to adjacent point has a stone occupied by AI Test 2-1:
	 * There is an adjacent to adjacent point that is empty Test 2-2: The adjacent
	 * point to that empty adjacent to adjacent point has a stone occupied by AI
	 * When all tests are passed, any of the two AI stones found can move to an
	 * empty point on the line AI can form a mill the turn after by moving the other
	 * stone to the last empty point on the line
	 * 
	 * @param pointToTest Point to check if a stone can make a logical choice to
	 *                    move here
	 * @param line        Line that pointToTest belongs to and will be called to
	...
	/**
	 * Case 3 of "two steps from mill" when AI can only move adjacent AI can move
	 * any stone on the mill to an adjacent empty point It can form a mill the turn
	 * after by moving the stone back to the line
	 * 
	 * @param pointToTest Point to check if a stone can make a logical choice to
	 *                    move here
	 * @param line        Line that pointToTest belongs to and will be called to
