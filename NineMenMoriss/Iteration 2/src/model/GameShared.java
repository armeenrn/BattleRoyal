...
public abstract class GameShared {
	...
	/**
	 * setGameConfig sets up the game. It creates the game board, creates the
	 ...
	 * 
	 * @return goFirst, the int determines if player1 or player2 goes first
	 */
	...
	/**
	 * getFilledLine returns the lines that are filled by three stones for each
	 * player
	 * 
	...
	/**
	 * getStonesOfOpponent returns the stones of the opponent player which can be
	 * moved if the other player has formed a line
	 * 
	...
	public void moveStone(Player player, Stone stone, Point point) {
		...
		/*
		 * If the player has less than 9 stones placed on the board, a new stone is
		 * placed
		 */
		...
		/*
		 * if the player has 9 stones on the board, then we begin moving stones on the
		 * board
		 */
	...
	/**
	 * getFreeStones returns the player's stones which they can move on the board
	 * after all 9 stones are placed on the board Player can only move free stones
	 * that are not part of a line, unless no free stones are left, in which all
	...
	public ArrayList<Stone> getFreeStones(Player player) {
		...
		/*
		 * Checking if each player's stone is part of a player's filled line on the
		 * board. If so, it is not considered a free stone
		 */
		...
		for (Stone eachStone : player.getStones()) {
			...
			/*
			 * if the stone is not part of any filled lines, it can be considered a free
			 * stone
			 */
		...
		/*
		 * if no free stones are available, then the player can move any stone on the
		 * board
		 */
	...
	/**
	 * checkIfPointIsUnOccupied checks if any player already has a stone on any
	 * selected point on the board
	 * 
	...
	/**
	 * checkIfAjdacent checks if a selected point is adjacent to the stone that is
	 * to be moved on the board
	 * 
	 * @param stoneToSelect, the stone that has been selected to be removed
	 * @param pointToSelect, the point that the stone has been selected to be moved
	 *                       to
	 * @return boolean indicating if this point is both adjacent to the stone and
					...
					try {
						if (eachLine.doesLineContain(stoneToSelect) == true) { /* checking if each line has the stone */
							if (eachLine.doesLineContainPoint(pointToSelect) == true) {
								...
								/*
								 * if the stone is on EndPoint1 of the line, we check if the point selected is
								 * on the adjacent midPoint
								 */
								...
								/*
								 * if the stone is on a midPoint of the line, we check if the point selected is
								 * on either adjacent end point
								 */
								...
								/*
								 * if the stone selected is on EndPoint2 of the line, we check if the point
								 * selected is on the adjacent midPoint
								 */
	...
	/**
	 * getAdjacentPoints returns an ArrayList of the available adjacent points that
	 * a particular stone can be moved to
	 * 
	 * @param stoneToMove, the stone that has been selected to be moved
	 * @return adjacentPoints, an ArrayList of the available points that are
	 *         adjacent to that selected stone
	 */
